...
as provide by segmentation algorithms or after connected component labeling of a binary image.
The library currently allows for computing different kind of features:
...
Plugin usage and definitions of the different features are described in the 
[user manual](https://github.com/ijtools/RegionFeaturesJ/releases/download/v1.0/regionFeatures_manual.pdf).

The library also makes it possible to manage features programmatically via the `RegionFeatures` class. 
Features can be added programmatically, and the resulting values are be concatenated into the
ResultsTable in the order specified by the addition order.
It is also possible to design its own feature(s), possibly based on other features, 
and to include them into the resulting table.

### Installation

Simply put the `RegionFeaturesJ.jar` into the plugins directory of the current ImageJ/Fiji installation,
and restart ImageJ/Fiji.
...
All plugins requires an image containing a **label map**, i.e. an image (8-bits, 16-bits, or 32-bits) 
containing for each pixel the integer label of the region it belongs to.
The behavior of the dialogs are similar to that of the "Analyze Regions (2D/3D)" Plugins in MorphoLibJ, with some enhancements:

* better dependency management between the features,
* better managerment of calibration units (that can be included in column names, as new columns, or as new table)
* possible inclusion of image name to facilitate analysis of image collections
* iteration over channels / frames / and possibly slices of a mutli-dimensional image

In addition, two plugins are provided to facilitate the interpretation of the results:

* Plugins -> IJ Tools -> RegionFeaturesJ -> Region Feature Overlay: display a geometric feature as an overlay over an image. Features corresponding to each region are displayed with a color specific to the region.
* Plugins -> IJ Tools -> RegionFeaturesJ -> Label Map Color Image: creates a color image representing the regions with the same colors used for the "Region Feature Overlay" plugin.
